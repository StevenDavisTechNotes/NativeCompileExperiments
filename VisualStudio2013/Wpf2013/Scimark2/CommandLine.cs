...
namespace SciMark2
{
	...
	public class CommandLine
	{
		...
		/// </summary>
		//[STAThread]
		public static void  submain(System.String[] args)
		{
			...
			if (args.Length > 0)
			{
				...
				{
					Debug.WriteLine("Usage: [-large] [minimum_time]");
					return;
				}
			...
			Debug.WriteLine("**                                                               **");
			Debug.WriteLine("** SciMark2a Numeric Benchmark, see http://math.nist.gov/scimark **");
			Debug.WriteLine("**                                                               **");
			...
			Debug.WriteLine("Mininum running time = {0} seconds", min_time);
			...
			res[0] = (res[1] + res[2] + res[3] + res[4] + res[5]) / 5;
		        ...
		        File.AppendAllText("ResultLog.txt", string.Format("Wpf2013,{0:F2}\n", res[0]));
		    }
			
			// print out results
			Debug.WriteLine("");
			Debug.WriteLine("Composite Score: {0:F2} MFlops", res[0]);
			Debug.WriteLine("FFT            : {0} - ({1})", res[1] == 0.0 ? "ERROR, INVALID NUMERICAL RESULT!" : res[1].ToString("F2"), FFT_size);
			Debug.WriteLine("SOR            : {1:F2} - ({0}x{0})", SOR_size, res[2]);
			Debug.WriteLine("Monte Carlo    :  {0:F2}", res[3]);
			Debug.WriteLine("Sparse MatMult : {2:F2} - (N={0}, nz={1})", Sparse_size_M, Sparse_size_nz, res[4]);
			Debug.WriteLine("LU             : {1} - ({0}x{0})", LU_size, res[1] == 0.0 ? "ERROR, INVALID NUMERICAL RESULT!" : res[5].ToString("F2"));
		}
	}
}